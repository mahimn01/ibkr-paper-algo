#!/usr/bin/env python3
"""
Commodity Momentum vs SPY Comparison

Direct comparison of commodity momentum strategy vs S&P 500 buy-and-hold.
Shows year-by-year and month-by-month performance side-by-side.
"""
...
def main():
    ...
    print("="*80)
    print(" "*15 + "COMMODITY MOMENTUM vs S&P 500 COMPARISON")
    print("="*80)

    # Commodity universe
    commodities = ["GLD", "SLV", "PPLT", "USO", "UNG", "DBA", "CORN", "WEAT", "DBC", "GSG"]

    print(f"\nFetching 3 years of data...")
    print(f"  - SPY (S&P 500)")
    print(f"  - {len(commodities)} commodity ETFs")

    # Connect to IBKR
    config = IBKRConfig(host="127.0.0.1", port=7497, client_id=21)
    ...
    print(f"✓ Commodities: {len(all_data)} loaded")

    # Align commodity data
    ...
    print(f"\nRunning commodity momentum backtest...")
    ...
    print(f"\n{'Metric':<25} {'Commodity Momentum':>20} {'SPY (S&P 500)':>20} {'Difference':>15}")
    print("-"*80)
    ...
    print(f"{'='*80}")
    print(f"{'Year':<6} {'Commodity Momentum':>20} {'SPY (S&P 500)':>20} {'Outperformance':>20}")
    print("-"*80)
    ...
    print(f"{'Month':<6} {'Commodity':>12} {'SPY':>12} {'Diff':>12}")
    print("-"*45)
    ...
    if alpha > 0:
        print(f"✓ Commodity Momentum OUTPERFORMED SPY by {alpha:.2f}% annually")
    else:
        print(f"✗ Commodity Momentum UNDERPERFORMED SPY by {abs(alpha):.2f}% annually")

    print(f"\nKey Insights:")
    print(f"  - SPY had {spy_stats['total_return']:.2f}% total return over period")
    print(f"  - Commodity strategy had {strategy_total:.2f}% total return")
    print(f"  - Commodity strategy had {strategy_max_dd:.2f}% max drawdown vs {spy_stats['max_dd']:.2f}% for SPY")
